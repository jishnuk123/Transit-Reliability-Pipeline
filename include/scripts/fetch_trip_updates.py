...
Exposes fetch_and_write() as the main entry point, so this logic can
...
def flatten_feed(feed):
    """Convert the nested FeedMessage structure into a flat list of dictionaries."""
    records = []
...
def fetch_and_write():
    """
    Main entry point: fetch, decode, flatten, and write MBTA Trip Updates.
    Returns the filepath written, so callers (including Airflow tasks)
    can pass it along to the next step.
    """
